    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.todos.update", [$todo->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $todo->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time_due') ? 'is-invalid' : '' }}" type="text" name="time_due" id="time_due" value="{{ old('time_due', $todo->time_due) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('send_reminder') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="send_reminder" id="send_reminder" value="1" {{ $todo->send_reminder || old('send_reminder', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('completed') ? 'is-invalid' : '' }}" type="number" name="completed" id="completed" value="{{ old('completed', $todo->completed) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('session') ? 'is-invalid' : '' }}" name="session_id" id="session_id" required>
                    @foreach($sessions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('session_id') ? old('session_id') : $todo->session->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
