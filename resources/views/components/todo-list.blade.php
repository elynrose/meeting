@if(!$todos->isEmpty())
    ...
    @can('todo_delete')
                    <form class="small pull-right" action="{{ route('frontend.todos.destroy', $todo->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;" class="pull-right">
    ...
    @endif
        ...
        <input type="text" name="item" class="form-control col-md-12" placeholder="Add a new task" required>
            @csrf
            <input type="date" name="due_date" value="" class="form-control date">
            <input type="hidden" name="completed" value="0">
            <input type="hidden" name="ordering" value="{{ $todo->ordering + 1 }}">            
        ...
        <div class="input-group mt-3">
            <textarea name="note" id="note" class="form-control" placeholder="Your notes here"></textarea>
        </div>
    </form>